...
public sealed partial class MainCycle_WhoHeroes
{
    ...
    private void UpdateNightOrchestration()
    {
        ...
        if (nightViewMapCameraController != null && nightViewMapCameraController.enabled)
            nightViewMapCameraController.enabled = false;
    ...
    private IEnumerator BeginNightBattle()
    {
        ...
        try
        {
            ...
            nightBattleSpawner.IsDone = false;
            nightBattleController.startDo = true;
            spawnRoutine = StartCoroutine(SpawnNightWaves());
            setupComplete = true;
        }
        finally
        {
    ...
    private void SpawnDemonBatch(RObj portal, Bon batch)
    {
        ...
        foreach (var unit in spawned)
        {
            ...
            unit.AddViz(DemonStateVisual);
            if (!unit.visuals.TryGetValue(DemonStateVisual, out var stateVisual) || stateVisual == null ||
                !stateVisual.TryGetComponent<WhoHeroesDemonStateMachine>(out var stateMachine) ||
                !stateMachine.Initialize(
                    unit, gateTarget, prince, NightBattleMeta, PrinceMeta,
                    nightDemonAggroRange, nightCastleGateRadius, NightDemonDecisionInterval))
            {
                Debug.LogError($"WhoHeroes night: demon state machine was not initialized for '{unit.RID}'.", this);
    ...
    private void SpawnDefense()
    {
        ...
        foreach (var entry in stacks)
        {
            ...
            var anchor = nightDefenseRows[rowIndex];
            var spawned = nightBattleSpawner.DoSpawnAny(
                new List<Bon> { new Bon { Key = entry.stack.dbObj.ID, Value = amount, Val3 = level } },
                "player", null, null, true, anchor.position, anchor.position,
                overridesViz: NightBattleVisualOverrides);
            ...
            PositionFormation(entry.stack, rowIndex, false);
            foreach (var unit in spawned)
            {
    ...
    {
        if (!nightActive || nightCleaningUp || MainCycle_WhoHeroes.Instance == null ||
            MainCycle_WhoHeroes.Instance.Phase != WhoHeroesPhase.Night)
            return;
    ...
    private void FocusNightCamera()
    {
        ...
        var position = nightCastleGate.position;
        position.z = savedCameraPosition.z;
        nightCamera.transform.position = position;
