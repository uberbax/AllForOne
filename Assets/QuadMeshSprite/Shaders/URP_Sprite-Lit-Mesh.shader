Shader "Universal Render Pipeline/2D/Sprite-Lit-Mesh"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            Varyings CombinedShapeLightVertex(Attributes v)
            {
                ...
                o.positionCS = TransformObjectToHClip(v.positionOS * _Flip);
                #if defined(DEBUG_DISPLAY)
            ...
            {
                const half4 mask = SAMPLE_TEXTURE2D(_MaskTex, sampler_MaskTex, i.maskUV);
                clip(mask.a - _Clip);

                const half4 main = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, i.uv);
                half4 y1 = 2 * main * _Color*2;
                half4 y2 = -(1 - 2 * main) + (1 - main) * 2 * _Color*2;
                half4 c = saturate((1 - _Color)* y1 + (_Color * y2));
                c.a = main.a * _Color.a;
