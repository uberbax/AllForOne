...
// Modified from my original version to have resizable bounds
...
namespace AnyPath.NativeTrees
{
        ...
        where T : unmanaged 
    {
        ...
        private NativeReference<AABB> bounds;
        private float3 boundsCenter => bounds.Value.Center;
        private float3 boundsExtents => bounds.Value.Size / 2;
        private float3 boundsQuarterSize => bounds.Value.Size / 4;
        ...
        public NativeOctree(AABB bounds, int objectsPerNode, int maxDepth, Allocator allocator, int initialCapacity = 0)
        {
            ...
            nodes = new NativeHashMap<uint, int>(initialCapacity / objectsPerNode, allocator);
  
            this.objectsPerNode = objectsPerNode;
            this.maxDepth = maxDepth;
            this.bounds = new NativeReference<AABB>(bounds, allocator);
        }
        ...
        /// Bounds of the tree
        /// </summary>
        public AABB Bounds => bounds.Value;
        ...
        public void CopyFrom(NativeOctree<T> source)
        {
            ...
            bounds.Value = source.bounds.Value;
            objects.Clear();
            nodes.Clear();
        ...
        {
            return JobHandle.CombineDependencies(nodes.Dispose(inputDeps), objects.Dispose(inputDeps), bounds.Dispose(inputDeps));
        }
